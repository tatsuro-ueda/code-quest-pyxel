...
flowchart TB
    subgraph BEFORE["Before（現状）"]
        ...
        B2 --> B_END["もとのままばんに変更が混ざり、CJ31 の遊び比べが崩れる"]
    end
...
- 承認後にだけ current 側へ昇格する流れを、CJ31/CJ32 の仕様どおりに整理する
...
- `test/test_build_web_release.py`
  preview 前提条件、昇格、selector -> wrapper -> html の導線をテストで固定し、CJ31/CJ32 の「混ぜない」を回帰させない
...
**Observe**：通常 build を実行すると変更済み成果物が `pyxel.html` 側へ出てしまい、CJ31 の「おためしばんで確認してから承認する」流れとずれていた。今回は時間優先で preview 側へファイルを退避して応急対応した。
...
**Think**：次回は J38 の `カスタマージャーニーgherkin` から再開し、`make build` / `--preview` / `--promote` の責務分離を仕様に沿って整理するのが最短。
**Act**：再開ポイントをノートに追記した。
