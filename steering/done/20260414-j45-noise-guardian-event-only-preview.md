...
**Act**：`main_preview.py` にだけ除外ロジックを入れ、`test/test_preview_noise_guardian.py` を追加して「zone 2 の通常遭遇に含まれない」「`_start_noise_guardian_battle()` は従来どおり起動する」を固定した。合わせて `preview_meta.json` に `つうしんとうの ノイズガーディアンが フィールドに でない` を記録し、`docs/product-requirements-guardrails.md` に専用 guardrail を追加した。
