...
- 分類: 開発（子どもが）/ デバッグ（親がAIで対応）/ 演出（ゲームには必要）/ 共有（ループを広げる）/ 発展（好循環を重ねる）/ 承認（主体性をシステムで守る）/ ガードレール（AIが壊すのを避ける）
- 正式ID: `CJxx` をカスタマージャーニーIDとして使う。古い note にある `Jxx` は旧表記。
- 対応するカスタマージャーニーgherkin は `CJGxx` を使う。`CJ` が「どんな体験を作りたいか」を語り、`CJG` は `Feature / Rule / Scenario` で「何を約束し、何を壊してはいけず、どう確かめるか」を語る。
...
| # | 分類 | タイトル | ジョブ | 構成 |
|---|---|---|---|---|
| CJ35 | ガードレール | AIで修正したらエラーが出て動かない | Job以前 | ガードレール |
| CJ36 | ガードレール | データを変えたらバランスが崩壊した | Job以前 | ガードレール |
| CJ37 | ガードレール | 責務が曖昧で直すほど別の所が壊れる | Job以前 | ガードレール |
| CJ38 | ガードレール | 新しいイベントを追加したら既存が壊れた | Job以前 | ガードレール |
| CJ39 | ガードレール | システムを変えたらゲーム全体が壊れた | Job以前 | ガードレール |
| CJ40 | ガードレール | ゲームモードを追加したら収拾がつかなくなった | Job以前 | ガードレール |
| CJ41 | ガードレール | 技術基盤を変えたら配信できなくなった | Job以前 | ガードレール |
| CJ23 | 開発 | スプライトを自分で描く | Job2: 好循環, Job3: アウトプット品質 | パイプライン |
| CJ24 | 開発 | 効果音を自分で作る | Job2: 好循環, Job3: アウトプット品質 | パイプライン |
| CJ01 | 開発 | はじめてのタイル配置 | Job2: 好循環 | パイプライン |
| CJ02 | 開発 | 道を作る | Job2: 好循環 | パイプライン |
| CJ03 | 開発 | 森を作る | Job2: 好循環 | パイプライン |
| CJ04 | 開発 | 水辺を作る | Job2: 好循環 | パイプライン |
| CJ05 | 開発 | 装飾で世界を彩る | Job2: 好循環 | パイプライン |
| CJ06 | 開発 | 迷路を作る | Job2: 好循環 | パイプライン |
| CJ07 | 開発 | ランドマークを配置する | Job2: 好循環 | パイプライン |
| CJ08 | デバッグ | 敵が強すぎる | Job1: コミュニケーション, Job2: 好循環 | — |
| CJ09 | デバッグ | セリフを変えたい | Job1: コミュニケーション, Job2: 好循環 | — |
| CJ10 | デバッグ | 新しい敵を追加したい | Job2: 好循環 | — |
| CJ12 | デバッグ | 歩いたら壁にハマった | Job1: コミュニケーション, Job2: 好循環 | — |
| CJ13 | デバッグ | 新しい呪文がほしい | Job1: コミュニケーション, Job2: 好循環 | — |
| CJ14 | デバッグ | マップが広すぎて迷う | Job2: 好循環 | — |
| CJ15 | 演出 | フィールドBGMをゾーンごとに付ける | Job3: アウトプット品質 | — |
| CJ16 | 演出 | 戦闘BGMを付ける | Job3: アウトプット品質 | — |
| CJ17 | 演出 | 効果音をイベントに紐づける | Job3: アウトプット品質 | — |
| CJ18 | 演出 | ダメージ演出を付ける | Job3: アウトプット品質 | — |
| CJ19 | 演出 | 場面転換の演出 | Job3: アウトプット品質 | — |
| CJ20 | 演出 | 演出ON/OFFで違いを体験する | Job1: コミュニケーション, Job3: アウトプット品質 | — |
| CJ27 | 発展 | ストーリーの分岐を作る | Job1: コミュニケーション, Job2: 好循環 | ガードレール |
| CJ28 | 発展 | 新しいエリアをまるごと追加する | Job2: 好循環, Job3: アウトプット品質 | ガードレール |
| CJ29 | 発展 | 全体のバランスを調整する | Job2: 好循環 | ガードレール |
| CJ30 | 発展 | エンディングを自分たちで書く | Job1, Job2, Job3 すべて | ガードレール |
| CJ42 | 発展 | 子どもが冒険を最後までやり切れる | Job2: 好循環 | ガードレール |
| CJ31 | 承認 | 子どもが変更を承認する | Job1: コミュニケーション, Job2: 好循環 | キュー |
| CJ32 | 承認 | 子どもが変更を却下する | Job1: コミュニケーション | キュー |
| CJ33 | 承認 | 子どもが変更を選んで適用する | Job1: コミュニケーション, Job2: 好循環 | キュー |
| CJ34 | 承認 | 承認したあとに「やっぱり」となる | Job1: コミュニケーション, Job2: 好循環 | キュー |
| CJ25 | 承認 | 親子で役割を交代する | Job1: コミュニケーション | キュー |
| CJ26 | 承認 | 「自分たちのゲーム」と言えるようになる | Job2: 好循環, Job3: アウトプット品質 | キュー |
| CJ21 | 共有 | 友達に見せる | Job2: 好循環, Job3: アウトプット品質 | — |
| CJ43 | 共有 | 実公開で遊ばれた記録が見える | Job1: コミュニケーション, Job2: 好循環 | — |
| CJ22 | 共有 | 友達のフィードバックを反映する | Job1: コミュニケーション, Job2: 好循環, Job3: アウトプット品質 | — |
...
flowchart LR
    ...
    subgraph After["After"]
        AM["感情: 方針を言える -> 曲が付く -> 世界が立ち上がる<br/>タッチポイント: Musicエディタ, ゾーン切替, Run"]
        AM -.-> A1
        A1[親「草原は冒険っぽく、森は不気味にしたい」] --> A2[親子がMusicエディタで曲の雰囲気を整える]
        A2 --> A3[selector から今の候補へ音楽を取り込む]
...
flowchart LR
    ...
    subgraph After["After"]
        AM["感情: 切り替わる -> 緊張感が出る -> ボス曲も欲しくなる<br/>タッチポイント: Musicエディタ, エンカウント切替, Run"]
        AM -.-> A1
        A1[親「戦闘はテンポ速めで緊張感を」] --> A2[親子がMusicエディタで戦闘曲を整える]
        A2 --> A3[selector から音楽を取り込むとエンカウント時に切り替わる]
        A3 --> A4[子ども「うわ、戦闘っぽい！」]
...
flowchart LR
    ...
    subgraph After["After"]
        AM["感情: 音が付く -> 気持ちいい -> 好みが見えてくる<br/>タッチポイント: Soundエディタ, 攻撃/回復/レベルアップ音, Run"]
        AM -.-> A1
        A1[親「攻撃・回復・レベルアップに音をつけよう」] --> A2[親子がSoundエディタで各イベント用のSFXを作る]
        A2 --> A3[selector から取り込んで Run → 攻撃するとシュッ！回復でキラン！]
...
flowchart LR
    ...
    subgraph After["After"]
        AM["感情: 並べられる -> 動き出す -> もっと具体化したくなる<br/>タッチポイント: Tilemap, AI追加, Musicエディタ, テストプレイ"]
        AM -.-> A1
        A1[「雪のエリアがほしい！」] --> A2[子どもがTilemapで白いタイルを並べる]
        A2 --> A3[親がAIに「雪ゾーンの敵とエンカウント率を追加して」]
        A3 --> A4[親子が Code Maker の Music エディタで雪エリアのBGMを決める]
        A4 --> A5[数十分で「雪エリア」が動く → テストプレイ]
    end
```
...
flowchart LR
    ...
    subgraph After["After: 遊び比べて判断"]
        AM["感情: 遊び比べられる -> 体感差がわかる -> 自分で決められる<br/>タッチポイント: 選択ページ, 開発版, 本番, 比較プレイ"]
        AM -.-> A1
        A1["承認画面を開く"] --> A2["「開発版」で遊ぶ\nスライムHP=30で戦闘"]
        A2 --> A3["「2かいで たおせた！」"]
        A3 --> A4["「本番」で遊ぶ\nスライムHP=50で戦闘"]
        A4 --> A5["「こっちは 4かい かかる」"]
        A5 --> A6["「開発版のほう！」→ 承認"]
...
flowchart LR
    ...
    subgraph After["After: 体感で却下できる"]
        AM["感情: 試してつまらないと感じる -> 前の方がいいと言える -> 却下できる<br/>タッチポイント: 開発版, 本番, 却下フロー"]
        AM -.-> A1
...
flowchart LR
    subgraph Before["Before"]
        BM["感情: さわりたい -> でも持ち出せない -> 自分のゲーム感が弱い<br/>タッチポイント: index.html, 手作業ビルド, Code Maker"]
        BM -.-> B1
        B1["開発版を見て『これを Code Maker で さわりたい』"] --> B2["でも index.html には play しかない"]
        B2 --> B3["親が別コマンドや手作業で zip を用意する"]
        B3 --> B4["どの版を持ち出したかも曖昧になる"]
        B4 --> B5["『自分で変えられる』まで届かず終わる"]
    end
    subgraph After["After"]
        AM["感情: そのまま持ち出せる -> もっと自分のゲームになる -> 続きを作りたくなる<br/>タッチポイント: index.html の開発版カード, code-maker zip, Pyxel Code Maker"]
        AM -.-> A1
        A1["開発版を見て『これを Code Maker で さわりたい』"] --> A2["index.html の 開発版カードから zip を落とす"]
        A2 --> A3["公式 Pyxel Code Maker を開いて スプライトや音をさわる"]
        A3 --> A4["『ここも ぼくたちで かえた！』"]
        A4 --> A5["自分たちのゲーム感が強まり つづきを作りたくなる"]
    end
```
...
flowchart LR
    ...
    subgraph After["After: 壊れた版は子どもに届かない"]
        ...
        A4 --> A5{起動・主要操作が通るか?}
        A5 -->|NG| A6[開発版を出さない<br/>親にエラー要約を返す<br/>→ AIに再依頼]
        A5 -->|OK| A7[選択ページに「開発版」が並ぶ]
...
```mermaid
flowchart LR
    subgraph Before["Before: 直すほど別の所が壊れる"]
        BM["感情: 頼む → 直ったと言われる → 他所が壊れて信用できない<br/>タッチポイント: AI修正依頼, 密結合コード, 別箇所バグ"]
        BM -.-> B1
        B1[親「ここを直して」] --> B2[AIが修正箇所と方法を検討]
        B2 --> B3[密結合されているが<br/>なんとか修正方法を決める]
        B3 --> B4[修正する]
        B4 --> B5[別箇所でバグが出る]
        B5 --> B6[親「また壊れた」<br/>好循環が鈍る]
    end
    subgraph After["After: 責務分担表に沿って直せる"]
        AM["感情: 頼む → 影響範囲が明確 → 安心して受け入れられる<br/>タッチポイント: AI修正依頼, 疎結合モジュール, 責務分担表"]
        AM -.-> A1
        A1[親「ここを直して」] --> A2[AIが修正箇所と方法を検討]
        A2 --> A3[疎結合された責務分担表に従い<br/>修正箇所と方法を決める]
        A3 --> A4[修正する]
        A4 --> A5[バグが出ない]
    end
```
