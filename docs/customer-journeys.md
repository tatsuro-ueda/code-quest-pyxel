...
| CJ23 | 開発 | ❌代筆→❤️所有感 | スプライトを自分で描く | Job2: 好循環, Job3: アウトプット品質 | パイプライン |
...
| CJ20 | 演出 | ❌一方的→❤️共通認識 | 演出ON/OFFで違いを体験する | Job1: コミュニケーション, Job3: アウトプット品質 | — |
...
**感情**：❌一方的（価値を言葉で説明しきれず先生化）→❤️共通認識（比べて一緒に考えたくなる）
...
flowchart LR
    ...
    subgraph After["After"]
        A1[（親）💡「音と光をオフにして遊んでみて」（対面）]
        A1 --> A2[（子ども）👆無音・演出なしでプレイ →「なんか寂しい…」（ゲーム中）]
        A2 --> A3[（親）👆ON に戻す → 子ども「全然違う！こっちがいい！」（ゲーム中）]
