...
├─ src/                       ソース正本
│  ├─ app.py                  BlockQuestApp（Phase 1 由来の薄い実行シェル。test/Code Maker bundler 用に残置）
│  ├─ game_data.py            YAML 生成物の読み出し口
│  ├─ core/                   Scene Protocol（test 専用、Game 本体は使わない）
...
### 4.2 `src/app.py`（legacy shell）

- **class BlockQuestApp**：Phase 1 で作った薄い実行シェル。`scene_manager` と `GameState` を保持し `update/draw` を委譲する。**現在の本体は `src/runtime/app.py::Game`** であり、`BlockQuestApp` は test／Code Maker bundler の互換と P1-G 時点の DI ハンドル目的で残置している（M4-3 完了時に整理候補）。

### 4.3 `src/game_data.py`
...
### 4.4 `src/core/scene_manager.py`（test 互換のみ）

- **class Scene(Protocol)**：`update / draw / name` を持つ最小 IF。
- **class SceneManager**（旧）：Scene オブジェクトを保持・履歴に積む。**現在の Game クラスは使わず**、`shared/services/scene_manager.py::SceneManager`（state holder、current/previous の 2 値）を採用している。本ファイルは test / `src/app.py::BlockQuestApp` 経由でのみ参照される。

### 4.5 `src/scenes/`（11 scene、MVP パターン）
...
### 4.6 `src/shared/services/`（18 サービス）
...
| `player_state.py` | legacy snapshot helper（M4-4 後の残存） | `dump_snapshot`, `restore_snapshot`, `exp_for_level`, `stats_for_level`。**player の正本は `src/shared/state/player_model.py::PlayerModel`**。`create_initial_player` などの dict 生成系は廃止済 |
...
| `item_use.py` | アイテム使用ロジック（M4-4 で PlayerModel に吸収予定） | `use_item` |
...
`main_development_runtime.py` は Phase 3 で削除済（2026-05-04）。`main_development.py` wrapper も同時に廃止し、配布物は `production/` 一本化済（→ memory `single_distribution_policy`）。
...
102 ファイルの pytest（717 tests）。`test_architecture_layout.py` が shared/services の配置・inline コピーの不在を検証し、`test_cjg_framework_rule_guards.py` が M1〜M5 規約の static guard（AGENTS≤100 行、architecture.md 存在、Game クラスの deprecated field 復活防止、SSoT 違反検出など）をまとめて持つ。
...
| `architecture.md`（本文書） | 人用詳細リファレンス（リポジトリ構造 + ディレクトリ規約 + 責務分担表） |
...
AI / 人ともに `AGENTS.md` → 必要なら `architecture.md`（本文書）→ 必要なら `framework-rule.md` の順に降りる 2 層構造。詳細は `framework-rule.md::M5-3` を参照。
...
**Phase 3 完了**：`main_development_runtime.py` / `main_development.py` wrapper / DevelopmentCandidate / `development/` 配布 / 承認ゲートを廃止、`production/` 一本化。`assets/blockquest.pyxres` が唯一の正本（→ memory `single_distribution_policy`）。
...
**M4-4 部分実施**：`PlayerModel` 新設、scenes 側の `player_state` shim 利用は PlayerModel 直呼びに統合済。`item_use.py` / `player_state.py` の残存は今後 PlayerModel 吸収予定。

**M5-3 完了**：AGENTS.md ≤100 行制約・docs/architecture.md / docs/framework-rule.md の 2 層文書ナビ確立。`docs/repository-structure.md` → `docs/architecture.md` リネーム。

**残タスク**：

| 項目 | 内容 |
|---|---|
| **M4-4 後半** | `item_use.py` / `player_state.py` を PlayerModel に吸収 |
| **`src/app.py` 整理** | Phase 1 由来の `BlockQuestApp` shell の最終整理（`runtime/app.py::Game` への一本化、または役割再定義） |
| **`src/core/scene_manager.py` 整理** | test 互換のためだけに残っているので、test 側を新 SceneManager 前提に書き換えて削除 |
