...
実装状況:
- `部分実装`: `index.html -> dist/play.html -> dist/pyxel.html` の共有導線、スマホ全画面ボタン、インストール不要のWeb配信はある
...
実装状況:
- `部分実装`: `dist/play.html` / `development/play.html` から `/internal/play-sessions` へ送る仕組みと SQLite 集計はある
...
Rule: 記録できていない状態を done 扱いしない
  ...
  Scenario: 公開サーバがログAPIを持たないなら見逃さない
    ...
    When dist/play.html または development/play.html が /internal/play-sessions へ送信する
...
Rule: Code Maker から戻す時は code を巻き戻さず必要な asset を取り込む
  ...
  Scenario: selector から Code Maker の zip を取り込める
    ...
    And 次に開く `dist/play.html` または `development/play.html` は取り込み後 asset を使う
...
Rule: 子どもに見える境界がある
  ...
  Scenario: Code Maker 教材版では編集可能領域が明示される
    ...
    When dist/code-maker.zip 内の main.py を開く
...
Rule: 新しい変更はまず開発版に入る
  ...
  Scenario: 親がAIに頼んだ変更はまず開発版に入る
    ...
    And 本番（dist/pyxel.html）は直前までに通った内容を保つ
