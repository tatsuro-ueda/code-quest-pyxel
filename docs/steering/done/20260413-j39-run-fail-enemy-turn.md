...
- **根拠となる journey**：`docs/gherkins/customer-journeys.md` の `J42: 子どもが冒険を最後までやり切れる`
- **関連する避けるジャーニー**：`docs/gherkins/customer-journeys.md` の `J39: システムを変えたらゲーム全体が壊れた`
...
- `J42` では `逃走失敗で敵ターンが来ない` 状態そのものが「RPGなのに最後まで遊べない」例として入っている
- `J39` では、戦闘ターン制御や行動順ロジックの破損がゲーム全体を不安定にする問題として定義されている
- `docs/gherkins/gherkin-guardrails.md` の `J39` には `Scenario: 逃走失敗後も戦闘ターンが継続する` があり、今回のバグはこの gherkin に直接対応する
...
- このバグは `J42` を壊し、`J39` の guardrail に反している不具合として扱う
...
- `docs/gherkins/gherkin-guardrails.md` `J39`
...
**Observe**：ユーザー報告どおり、戦闘で `にげる` に失敗しても敵が攻撃してこず、何度でも逃走を試せる状態になっている。これは `J42` に書いた「逃走失敗で敵ターンが来ない」そのものの症状で、`J39` の戦闘ターン制御破損にも当たる。  
...
**Act**：`J42` / `J39` / `Scenario: 逃走失敗後も戦闘ターンが継続する` を根拠に、逃走失敗バグ修正専用の task note を起票した。
