...
### 第0優先（土台）
0. **データ外部化（YAML化）** — `assets/enemies.yaml` `assets/items.yaml` `assets/spells.yaml` `assets/weapons.yaml` `assets/armors.yaml` `assets/exp_table.yaml` を作り、ローダを用意する
   - これを先にやるとその後の追加（敵12種、5呪文、武器7・防具7など）が単なるデータ追記で済む

### 第1優先（プレイ可能性）
1. **ショップ実装**（武器・防具・道具屋） — `SHOP_WIP_MSG` を本実装に置換
2. **武器・防具データの拡充**（YAMLに7種ずつ＋購入メッセージ）
3. **アイテム追加**（アンチウイルス・セーブポイント・毒システム）
4. **敵データ全12種をYAMLに移植**（魔王クローン・プロフェッサー含む）

### 第2優先（演出）
5. **SFX 21音** — `pyxel-rpg-sepack` を導入してマッピング
6. **スプラッシュ画面**
7. **ランドマーク強調描画**（世界樹・通信塔を目印として描き分け）

### 第3優先（深さ）
8. **魔法システム**（5呪文 + レベル習得 + MP消費、データはYAML）
9. **多段階ボス**（`getBossPhases` 相当）
10. **世界樹インタラクション**（`interactWorldTree` + `landmarkTreeSeen` 連動）
11. **通信塔インタラクション**（`interactTower` + `queueTowerEpilogue` + `landmarkTowerSeen`）
12. **プロフェッサー戦＋真エンディング**（`enterProfessorEnding`/`drawProfessorEnding`/`completeProfessorEnding`）

### 第4優先（土台仕上げ）
13. **Tiled で全マップを編集可能化** — `docs/steering/20260406-TiledMapEditor/` の方針に沿って手続き生成からTMXへ寄せる
14. `createInitialPlayer/resetGameState` のリファクタ
...
- 第0優先の YAML 外部化を先に終わらせると、第1〜3優先がほぼデータ追記＋小ロジックで済み、進捗体感が一気に上がる
