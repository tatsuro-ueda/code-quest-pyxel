...
**Rule**: Pyxel の呼び出しと入力取得は、**View 層と最外殻（`src/platform/pyxel_runtime.py` 相当）** に閉じ込める。Model / Presenter / Service / shared/state では呼ばない。

**Why**: Pyxel 呼び出しが散ると、テスト時に差し替えが効かず Model がヘッドレスで動かせなくなる。入力取得が散ると、再現性とテスト性が一気に落ちる。1 か所に絞れば、見た目や入力仕様を直したいときにその 1 か所だけ直せば済む。
...
* `models`（Pyxel 呼び出しは View または最外殻へ）
* `presenters`（同上）
* `services`（同上。ただし Audio/Save の Pyxel 依存ラッパは別）
* `shared/state`（同上）
...
- `grep -nE 'pyxel\.' src/` の結果が `src/scenes/*/view.py` と `src/platform/` / `src/runtime/` 相当の最外殻に限定される
- `grep -nE 'pyxel\.(btnp|btn)' src/scenes/*/presenter.py src/scenes/*/model.py src/shared/state/*.py` が 0 件
...
- `grep -nE 'pyxel\.' src/scenes/*/model.py src/shared/state/*.py` が 0 件
