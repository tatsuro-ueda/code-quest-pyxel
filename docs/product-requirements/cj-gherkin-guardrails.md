...
| 2. コンテンツ・演出 | CJG37 | タイル、スプライト、BGM、SE | pyxresとコードの乖離 |
| 3. イベント・ロジック | CJG38 | 会話、フラグ分岐、クエスト | 既存フラグとの衝突 |
| 4. システム・ルール | CJG39 | 戦闘、移動、装備、成長 | モノリス内の連鎖破壊 |
| 5. メタ構造 | CJG40 | ゲームモード、進行構造、周回 | セーブデータ互換崩壊 |
...
**スコープ**: CJG35の責務は**起動クラッシュの検出のみ**。「動くけどバランスが壊れている」「動くけど見た目がおかしい」といった意味的な誤りはCJG35では検出しない。それらは各レイヤー（CJG36-CJG41）のガードレールが担う。
...
`CJ42: 子どもが冒険を最後までやり切れる` のうち、「進行が止まらず最後までつながる」部分を主にここで守る。
...
Scenario: 新規イベント追加後に主要シナリオが走破できる
  ...
  Then 「フィールド探索→通常戦闘→経験値獲得→成長確認→ボス撃破→エンディング」の
      主要パスが例外なく走破される
...
`CJ42: 子どもが冒険を最後までやり切れる` のうち、「戦闘・成長・操作がRPGとして破綻しない」部分を主にここで守る。
...
Scenario: 逃走失敗後も戦闘ターンが継続する
  Given AIが戦闘の逃走ロジックまたは行動順ロジックを変更した
...
Scenario: イベント専用の敵は通常エンカウントに混ざらない
  Given AIが敵データまたは通常エンカウント候補のロジックを変更した
  And 一部の敵はイベント専用フラグを持つ
  When zone ごとの通常エンカウント候補を組み立てる
...
|---|---|---|---|---|
| G1 | `src/generated/` 直接編集禁止 | CJG36 データ | PreToolUse hook + chmod 444 | ツール拒否＋再指示（OS権限でも拒否） |
| G2 | `*.pyxres` 直接編集禁止 | CJG37 コンテンツ | PreToolUse hook | ツール拒否 |
| G3 | SSoT編集後の自動Codegen + テスト | CJG36 データ | PostToolUse hook | 自動再生成 → pytest 130テスト実行、失敗時は停止 |
| G4 | TILE_DATA ⇔ pyxres 整合チェック | CJG37 コンテンツ | `make build` | 不一致なら自動再生成 |
| G5 | イメージバンク番号の競合検出 | CJG37 コンテンツ | PostToolUse lint | 警告 |
| G6 | サウンド初期化順序の検証 | CJG37 コンテンツ | `make build` | ビルド失敗 |
| G7 | フラグ名の重複検出 | CJG38 イベント | PostToolUse lint | 警告＋一意名を促す |
| G8 | ヘッドレス起動テスト | CJG35 全レイヤー | `make build` | 承認キューから除外 |
| G9 | ヘッドレスシナリオテスト（4シナリオ） | CJG38-CJG39（CJ42縦断体験を含む） | `make build` | 承認キューから除外 |
| G10 | セーブデータ互換テスト | CJG40 メタ構造 | `make build` | 承認キューから除外 |
| G11 | Web版ヘッドレステスト | CJG41 技術基盤 | `make build` | 承認キューから除外 |
| G12 | Code Maker 互換テスト | CJG41 技術基盤 | `make build` | ビルド失敗 |
| G13 | 生成物の手編集検出 | CJG36 データ | `make build` 内 git diff | ビルド失敗 |
| G14 | 直接import禁止（ローダ経由強制） | CJG36 データ | PreToolUse hook + lint | ツール拒否 |
...
Feature: AIの改変で既存の挙動が壊れたらテストが検出する
  hookで防げるのは「間違ったファイルへのアクセス」だけ。
  正しいファイルを正しく編集しても、ロジックが壊れることがある。
  テストコードで重要な挙動を固定し、壊れたら即座に気づけるようにする。

Background:
  Given ゲームの重要な挙動はテストコードで固定されている
  And テストは pytest で実行される

Scenario: データ改変後にゲームデータの整合性が保たれている
  Given AIが enemies.yaml や spells.yaml を編集した
  When pytest が実行される
  Then 敵・呪文・アイテム等のデータが正しくロードされる
  And 参照先（ドロップアイテムID、呪文ID等）が全て存在する

Scenario: ロジック改変後に戦闘・移動の基本挙動が維持されている
...
Scenario: セリフ改変後にダイアログ構造が壊れていない
  ...
  When pytest が実行される
  Then 全シーンに必要なキー（speaker, lines等）が存在する
  And 参照されるシーンIDが全て定義されている
...
|---|---|---|---|---|
| 1. ボスのステータス定義 | CJG36 データ | assets/enemies.yaml に追加 | — | G1, G3: SSoT + 整合チェック |
| 2. ボスのスプライト描画 | CJG37 コンテンツ | **できない** → バンク座標を案内 | Image エディタで描画 | G2: pyxres直接編集ブロック |
| 3. ボス出現イベント | CJG38 イベント | main.py にイベント追加 | — | G7: フラグ重複検出, G9: シナリオテスト |
| 4. ボス専用戦闘ロジック | CJG39 システム | main.py に戦闘パターン追加 | — | G9: 戦闘シナリオ完走テスト |
| 5. 起動確認 | CJG35 全体 | — | — | G8: ヘッドレス起動テスト |
...
| ステップ | レイヤー | AI | 人間(Code Maker) | ガードレール |
|---|---|---|---|---|
| 1. 呪文データ定義 | CJG36 データ | assets/spells.yaml に追加 | — | G1, G3: SSoT + 整合チェック |
| 2. エフェクト演出 | CJG37 コンテンツ | コード上の色・パターンで表現 | （派手にしたければ Image エディタ） | G4: pyxres整合 |
| 3. SE追加 | CJG37 コンテンツ | **できない** → スロット番号を案内 | Sound エディタで作成 | G2, G6: pyxres + 初期化順序 |
| 4. 習得条件・戦闘UI | CJG39 システム | main.py のレベルテーブル・戦闘UI更新 | — | G9: 戦闘シナリオ完走テスト |
...
| ステップ | レイヤー | AI | 人間(Code Maker) | ガードレール |
|---|---|---|---|---|
| 1. セリフ変更 | CJG36 データ | assets/dialogue.yaml を編集 | — | G1, G3: SSoT + 整合チェック |
| 2. 新イベント追加 | CJG38 イベント | main.py にフラグ分岐追加 | — | G7: フラグ重複検出 |
| 3. イベント内でセリフ参照 | CJG36 + CJG38 | 生成されたセリフIDをイベントで参照 | — | G3: 整合チェックで参照先存在を検証 |
...
| ステップ | レイヤー | AI | 人間(Code Maker) | ガードレール |
|---|---|---|---|---|
| 1. 敵ステータス変更 | CJG36 データ | assets/enemies.yaml を編集 | — | G1, G3 |
| 2. ドロップアイテム変更 | CJG36 データ | assets/items.yaml + enemies.yaml を編集 | — | G3: アイテムID存在チェック |
| 3. バランス確認 | CJG36 データ | — | — | G9: 戦闘シナリオ（倒せるか） |
...
5. **CJG35のスコープ**：起動クラッシュの検出のみ。「動くけど壊れている」はCJG36-CJG41の責務。
