...
好循環が速く回るほど、親が「しゃしゃり出す」リスクも上がる。AIで何でも数分で直せるから、子どもに聞かずに先回りしてしまう。ここでは、新しい変更をまず `おためしばん` として見せ、次の依頼が来るまでは current / preview を遊び比べられるようにして、この問題をシステムで防ぐ。ハンドルを子どもが握り続けるためのブレーキは、「まず遊んで、いやなら止められる」ことにある。

### CJ31: 子どもが変更を承認する

親がAIに頼んだ修正を、子どもが「おためし」と「もとのまま」の両方で遊び比べて、自分の体感で「こっち！」と決める。親がAIに変更を頼んだら、その変更の受け皿は常に preview（おためしばん）であり、current（もとのままばん）は直前までに通った内容を配る。`おためしばん` は原則として次に通る版で、次の依頼が来たら前の `おためしばん` は current に繰り上がり、新しい依頼の差分が新しい `おためしばん` になる。ただし、次の依頼に対応する preview をまだ作っていないなら、前の依頼で作った preview をそのまま新しい依頼の `おためしばん` として見せてはいけない。選択ページの説明は、今の依頼に対応する preview に実際に入っている変更から自動で出て、「ここを変えたから、遊んでみて！」と子どもに伝わる形でなければならない。
...
子どもが両方遊んだ結果「まえのがいい」と自信を持って却下する。親の理屈に言い返せなくても、遊んだ体感が根拠になる。ここは「おためしばんは原則として通る」という流れを止める、明示的な例外である。却下したら current をそのまま保ち、おためしばんは取り下げられる。
...
親がAIに複数の修正を頼んだとき、子どもがどれを採用するか・どの順で入れるかを選ぶ。優先順位を決めるのは子ども。選択ページの変更一覧は、親があとから別に手で説明を書くものではなく、その時点の preview に入っている変更から自動生成され、実際に遊べる変更と一対一に対応している必要がある。ここでいう一覧は「今の依頼に対応する `おためしばん`」だけを説明するもので、前の依頼で通った preview は current 側へ繰り上げ、新しい候補の説明へ混ぜてはいけない。
...
| # | 分類 | タイトル | ジョブ |
|---|---|---|---|
| CJ01 | 作る | はじめてのタイル配置 | Job2: 好循環 |
| CJ02 | 作る | 道を作る | Job2: 好循環 |
| CJ03 | 作る | 森を作る | Job2: 好循環 |
| CJ04 | 作る | 水辺を作る | Job2: 好循環 |
| CJ05 | 作る | 装飾で世界を彩る | Job2: 好循環 |
| CJ06 | 作る | 迷路を作る | Job2: 好循環 |
| CJ07 | 作る | ランドマークを配置する | Job2: 好循環 |
| CJ08 | 直す | 敵が強すぎる | Job1: コミュニケーション, Job2: 好循環 |
| CJ09 | 直す | セリフを変えたい | Job1: コミュニケーション, Job2: 好循環 |
| CJ10 | 直す | 新しい敵を追加したい | Job2: 好循環 |
| CJ11 | 直す | BGMの雰囲気を変えたい | Job2: 好循環 |
| CJ12 | 直す | 歩いたら壁にハマった | Job1: コミュニケーション, Job2: 好循環 |
| CJ13 | 直す | 新しい呪文がほしい | Job1: コミュニケーション, Job2: 好循環 |
| CJ14 | 直す | マップが広すぎて迷う | Job2: 好循環 |
| CJ15 | 磨く | フィールドBGMをゾーンごとに付ける | Job3: アウトプット品質 |
| CJ16 | 磨く | 戦闘BGMを付ける | Job3: アウトプット品質 |
| CJ17 | 磨く | 効果音をイベントに紐づける | Job3: アウトプット品質 |
| CJ18 | 磨く | ダメージ演出を付ける | Job3: アウトプット品質 |
| CJ19 | 磨く | 場面転換の演出 | Job3: アウトプット品質 |
| CJ20 | 磨く | 演出ON/OFFで違いを体験する | Job1: コミュニケーション, Job3: アウトプット品質 |
...
| CJ23 | 作る | スプライトを自分で描く | Job2: 好循環, Job3: アウトプット品質 |
| CJ24 | 作る | 効果音を自分で作る | Job2: 好循環, Job3: アウトプット品質 |
| CJ25 | 守る | 親子で役割を交代する | Job1: コミュニケーション |
| CJ27 | 育てる | ストーリーの分岐を作る | Job1: コミュニケーション, Job2: 好循環 |
| CJ28 | 育てる | 新しいエリアをまるごと追加する | Job2: 好循環, Job3: アウトプット品質 |
| CJ29 | 育てる | 全体のバランスを調整する | Job2: 好循環 |
| CJ30 | 育てる | エンディングを自分たちで書く | Job1, Job2, Job3 すべて |
| CJ42 | 育てる | 子どもが冒険を最後までやり切れる | Job2: 好循環 |
| CJ31 | 守る | 子どもが変更を承認する | Job1: コミュニケーション, Job2: 好循環 |
| CJ32 | 守る | 子どもが変更を却下する | Job1: コミュニケーション |
| CJ33 | 守る | 子どもが変更を選んで適用する | Job1: コミュニケーション, Job2: 好循環 |
| CJ34 | 守る | 承認したあとに「やっぱり」となる | Job1: コミュニケーション, Job2: 好循環 |
| CJ35 | 避ける | AIで修正したらエラーが出て動かない | Job2: 好循環（ブレーキ役） |
| CJ36 | 避ける | データを変えたらバランスが崩壊した | Job2: 好循環 |
| CJ37 | 避ける | 見た目・音を変えたら表示が壊れた | Job1: コミュニケーション, Job2: 好循環 |
| CJ38 | 避ける | 新しいイベントを追加したら既存が壊れた | Job2: 好循環 |
| CJ39 | 避ける | システムを変えたらゲーム全体が壊れた | Job2: 好循環 |
| CJ40 | 避ける | ゲームモードを追加したら収拾がつかなくなった | Job2: 好循環 |
| CJ41 | 避ける | 技術基盤を変えたら配信できなくなった | Job2: 好循環, Job3: アウトプット品質 |
