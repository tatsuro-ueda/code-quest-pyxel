...
- 対象ジャーニー: J12, J20-J23, J25-J26, J31-J34
...
```gherkin
Scenario: 親がアクセスログで遊ばれているか確認できる
...
```gherkin
Scenario: 親の修正がもとのまま版に影響しない
  Given 親がAIに変更を頼み、おためし版（pyxel-preview.html）をビルドした
  When 子どもがもとのまま版（pyxel.html）で遊ぶ
  Then ゲーム内容は変更前のまま動作する
```
...
```gherkin
Scenario: 遊び比べた後に承認すると変更が本番に反映される
  Given 子どもが両方のバージョンで遊んだ
  When 子どもが「おためしのほうがいい！」と親に伝える
  And 親がおためし版を昇格させる
  Then 次回のビルドでおためし版の内容がmain.pyとして配信される
```
...
Scenario: 選択ページの変更説明が実際の配信内容と一致する
  Given 親がAIに変更を頼んで build した
  When 子どもが選択ページの変更説明を読む
  Then 説明は実際に build された内容と一致している
  And 古い変更説明が残ったまま比較させない
```
...
Scenario: おためし版に含まれる変更一覧が表示される
  ...
  When 選択ページが表示される
  Then おためしばんカードに変更リストがひらがなで列挙される
  And 子どもが何が変わるか一覧で確認できる
```

```gherkin
Scenario: 変更一覧が古いままなら build で検知できる
  Given 実際に配信される変更と change list がずれている
  When 親が build する
