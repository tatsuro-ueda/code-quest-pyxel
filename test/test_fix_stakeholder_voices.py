...
class FixStakeholderVoicesTest(unittest.TestCase):
    def test_fix_cli_runs_clean_on_real_repo(self):
        completed = subprocess.run(
            [sys.executable, str(ROOT / "tools" / "fix_stakeholder_voices.py")],
            capture_output=True,
            text=True,
            cwd=ROOT,
            check=False,
    ...
    def test_run_fixer_normalizes_duplicate_requirement_lists(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            request = fixed_yaml["facts"]["requests"][0]
            requirement = fixed_yaml["facts"]["requirements"][0]

        self.assertEqual(result["status"], "FIXED")
        self.assertGreaterEqual(len(result["applied_fixes"]), 1)
        self.assertEqual(request["source_trace_refs"], ["framework_rule:M1"])
        self.assertEqual(request["source_refs"], ["docs/source.md"])
        self.assertEqual(requirement["derived_from_request_ids"], ["rq_dev_safe"])
        self.assertEqual(requirement["acceptance_ids"], ["acc_safe_architecture_boundaries_guard"])
        self.assertEqual(requirement["stakeholder_ids"], ["st_repo_developer"])
        self.assertEqual(requirement["source_trace_refs"], ["framework_rule:M1"])
        self.assertEqual(requirement["affected_paths"], ["src/app.py"])
        self.assertEqual(requirement["verification_refs"], ["test/test_one.py"])
        self.assertEqual(requirement["source_refs"], ["docs/source.md"])
